...
(pc/remove-facts! test-db "characters" test-atom
                [parent "???" "Chewbacca"]
                [parent "???" "Anakin Skywalker"]
                [male "???"]
                [male "Chewbacca"])
...
;; Try reloading the page and evaluting the default the template
;; and everything above it. Now check to see if the change we made
;; persisted.
(println (pm/with-db @test-atom
           (lm/run* [q]
             (male "Jar Jar Binks"))))
